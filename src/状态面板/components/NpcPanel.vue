    ...
    <div v-else class="npc-list">
      ...
      >
        <div class="card-header">
          ...
          <span class="npc-identity">{{ npc.identity }}</span>
          <span class="npc-power">⚡{{ npc.power }}</span>
        </div>
        <div class="card-summary">
          <span>❤️ {{ npc.hp.current }}/{{ npc.hp.max }}</span>
          <span>🧠 {{ npc.sanity.current }}/{{ npc.sanity.max }}</span>
          <span>情感: {{ npc.emotion_level }}</span>
        </div>
        ...
        <Transition name="expand">
          <div v-if="expandedIdx === idx" class="card-detail" @click.stop>
            <div class="detail-row">
              ...
              <span class="dv">{{ npc.physical_state }}</span>
            </div>

            <div class="detail-section">
              <span class="ds-title">装备</span>
              <div class="eq-row" v-for="eq in equipOf(npc)" :key="eq.key">
            ...
            <div class="detail-section">
              ...
              <div class="bp-row" v-for="bp in bodyOf(npc)" :key="bp.key">
...
function equipOf(npc: NpcData) {
  return [
    { key: 'weapon', label: '武器', value: npc.equipment.weapon },
    { key: 'accessory', label: '饰品', value: npc.equipment.accessory },
    { key: 'top', label: '上装', value: npc.equipment.top },
    { key: 'bottom', label: '下装', value: npc.equipment.bottom },
    { key: 'shoes', label: '鞋子', value: npc.equipment.shoes },
  ];
}

function bodyOf(npc: NpcData) {
  const b = npc.body;
  return [
...
.npc-name { font-size: 13px; font-weight: bold; color: #4a3728; }
.npc-identity { font-size: 10px; color: #6b5440; }
.npc-power { font-size: 10px; color: #c9a84c; font-weight: bold; }
